...
world = make_world(shape, randbaseint=0)
world2 = make_world(shape, randbaseint=0)

make_plot(world,title='Porosity', cmap='gray')
make_histogram(world, title="Porosity")  
save_img(world, h=shape[0], w=shape[1], cmap='gist_gray', title='generated_porosity.png')
...
velocity = np.sqrt(world**2 + world2 **2)
make_histogram(velocity, title="np.sqrt(world**2 + world2 **2)") 

kc_world = Kozeny_Carman(velocity)

make_histogram(kc_world, title="Raw KC permability")
make_plot(kc_world,title='Raw KC permability', cmap='gray')
