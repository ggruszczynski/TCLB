...
# AddDensity(name="omega_phi", group="porous_media", comment="omega_phi=1.0/(3*M+0.5) recommended value 0.2", parameter=TRUE)
AddDensity(name="poro0", group="porous_media", comment="initial_porosity", parameter=TRUE)
AddDensity(name="IB_permability", group="porous_media", comment="initial_permability_IB_coeff", parameter=TRUE)
...
if (Options$Outflow) {
	...
	# iteration
	AddStage("BaseIter"  , "calcHydroIter"      , save=Fields$group %in% c("g","h","Vel","nw","porous_media", "gold","hold"), 
	...
	# iteration
	AddStage("BaseIter"  , "calcHydroIter"      , save=Fields$group %in% c("g","h","Vel","nw","porous_media") , load=DensityAll$group %in% c("g","h","Vel","nw","porous_media"))  # TODO: is nw needed here?
...
AddQuantity(name="IBPermability", unit="1")
...
AddSetting(name="PF_counterdiff_coeff", default=1.0, comment='1 - counteract diffusion; 0 - allow PF to diffuse')
AddSetting(name="PF2NS_coupling", default=1.0, comment='1 as as in PF model; 0 makes grad(PF)=0 in NS')
...
if (Options$debug){
	...
	AddGlobal(name="F_DarcyX", comment='Darcy force X', unit="")
	AddGlobal(name="F_DarcyY", comment='Darcy force Y', unit="")
...
AddNodeType(name="DirichletPFEQ", group="ADDITIONALS_PF") # to make PF source
