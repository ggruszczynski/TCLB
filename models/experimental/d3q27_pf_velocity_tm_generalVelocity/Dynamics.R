...
AddSetting(name="R", default="0.0", comment='Poiseuille flow - pipe radius')
AddSetting(name="Center", default="0.0", comment='Poiseuille flow - center coordinate for pipe')
AddSetting(name="Poiseuille", default="0.0", comment='Turn on Poiseuille flow on boundary', zonal=T)
AddSetting(name="Uavg", default="0.0", comment='Average Poiseuille flow velocity = 0.5*umax')
AddSetting(name="myK", default="0.0", comment='For annular Poiseuille flow this is the inner to outer diameter ratio')
...
AddNodeType(name="GeneralVelocity", group="BOUNDARY")
if (Options$OutFlow){
