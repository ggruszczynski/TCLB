            ...
            with open(input_filepath, "r") as in_file:
                ...
                for quantity in quantities_of_interest:
                    for line in str.splitlines(content):
                        ...
                        if re.findall(" cm_TB_batch_Eo", line):
                            number = re.search(r"Eo(\d+)_Mo(.*)", line)
                            print(float(number.group(1)))
                            print(float(number.group(2)))
                            quantities_of_interest["Eo"] = float(number.group(1))
                            quantities_of_interest["Mo"] = float(number.group(2))
...
"""
parse_logs(input_dir=os.path.join(input_folder_path),
           output_file=os.path.normpath(os.path.join(input_folder_path,"./parsed_lines_cm.log")),
           quantities_of_interest=quantities)
