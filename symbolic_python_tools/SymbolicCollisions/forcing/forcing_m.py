...
print_as_vector(F_m_Guo_extended, 'F_m', regex=True)
...
print('\n//Force -> Force_m - from continous definition: \n'
      'k_mn = integrate(fun, (x, -oo, oo), (y, -oo, oo)) \n'
      'where fun = forceM(rho,u,x,y) *(x)^m (y)^n ')
F_m = get_mom_vector_from_continuous_def(get_continuous_force_Guo, continous_transformation=get_continous_m)
print_as_vector(F_m, 'F_m', regex=True)

print('\n//population_eq -> cm_eq - from continous definition: \n'
      'k_mn = integrate(fun, (x, -oo, oo), (y, -oo, oo)) \n'
      'where fun = forceM(rho,u,x,y) *(x)^m (y)^n ')
# cm_eq = get_cm_vector_from_continuous_def(get_continuous_Maxwellian_DF)
m_eq = get_mom_vector_from_continuous_def(get_continuous_hydro_DF, continous_transformation=get_continous_m)
...
F_m = get_mom_vector_from_continuous_def(get_continuous_force_He_MB, continous_transformation=get_continous_m)
print_as_vector(F_m, 'F_m', regex=True)
